...
class ConversationMemory:
    ...
    def _process_action(
        ...
        if isinstance(
            ...
            if (
                "claude-3-7" in llm_response.model
                and hasattr(llm_response.choices[0].message, "thinking_blocks")
                and llm_response.choices[0].message.thinking_blocks
            ):
                content += llm_response.choices[0].message.thinking_blocks
            content += [TextContent(text=assistant_msg.content or "")] if assistant_msg.content is not None else []
